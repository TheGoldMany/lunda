...
function pinIcon(emoji: string, variant: "primary" | "default" = "default") {
  return L.divIcon({
    html: `<span class="map-pin ${variant === "primary" ? "map-pin-primary" : ""}">${emoji}</span>`,
  ...
  return (
    <div className="map-view" style={{ height }}>
      ...
      >
        ...
        {markers.map((m) => (
          <Marker key={m.id} position={[m.latitude, m.longitude]} icon={pinIcon(m.emoji, m.variant)} title={m.label}>
